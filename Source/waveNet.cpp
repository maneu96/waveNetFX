...
void waveNet::loadLayers(String fileName){
   /* std::ifstream jsonStream("/Users/maneu/Downloads/updated_conv1d_network_parameters.json"); */
    ...
    if(jsonStream.is_open()){
        ...
        std::vector<std::vector<std::vector<float> > > weightsInputLayer (16,std::vector<std::vector<float> >(1,std::vector <float>(3,0)));
...
float waveNet::predict(Eigen::Matrix<float,N_SAMPLES,1> x){
...
void waveNet::hiddenLayerForward (uint index){
    ...
    if (index == 0){
       ...
       hiddenFilterActivation[0].forward(filter_1[0].outs);
        ...
        //
        //Saving the output
        Eigen::Map<Eigen::Matrix<float,N_CHANNELS*N_SAMPLES,1>> (zout.data() + N_CHANNELS*N_SAMPLES*(index-1),N_CHANNELS*N_SAMPLES)= outGateBuffer;
