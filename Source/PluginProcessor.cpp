...
{
  /*  std::ifstream jsonStream ("/Users/maneu/Downloads/ModelKeras.json", std::ifstream::binary);
    auto model = RTNeural::json_parser::parseJson<float>(jsonStream,true);
    float x = 1.0;
    float input[16] = {x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x};
    model->forward(input);
    
    */
   // auto layer = model->layers;
    //auto weights = layer->bias;
...
void DistFxWaveNetAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
       auto * x = buffer.getWritePointer (channel);
        ...
        {
            float input[] = {x[n]};
            //x[n] = volLinear * neuralNet->forward(input);
